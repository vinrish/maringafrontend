...
const { data: editData, execute: fetchEditData } = await useApi<any>(`/companies/${route.params.id}/edit`)
...
//   editData.value?.clients
//     ? editData.value.clients.map(client => ({
//       text: `${client.user.first_name || ''} ${client.user.middle_name || ''} ${client.user.last_name || ''}`.trim(),
//       value: client.id,
//     }))
//     : [],
// )
...
watch(editData, newEditData => {
  ...
  if (newEditData?.company) {
    const company = newEditData.company

    formData.name = company.name || ''
    formData.notes = company.notes || ''
    formData.postal_code = company.postal_code
    formData.country = company.country
    formData.county = company.county
    formData.town = company.town
    formData.website = company.website
    formData.industry = company.industry
    formData.revenue = company.revenue
    formData.email = company.email
    formData.phone = company.phone
    formData.employees = company.employees
    formData.client_id = company.client_id || null
    formData.reg_date = company.reg_date
    formData.reg_number = company.reg_number
    formData.kra_pin = company.kra_pin
    formData.fiscal_year = company.fiscal_year
    formData.kra_obligations = company.kra_obligations
...
const onSubmit = () => {
  ...
  $api(`/companies/${route.params.id}`, {
    ...
    alertMessage.value = 'Company updated successfully!'
